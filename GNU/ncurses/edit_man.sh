...
*.[0-9]*)
	...
	if test ! -f /usr/jens/fix/ncurses/man_alias.sed ; then
cat >>/usr/jens/fix/ncurses/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /usr/jens/fix/ncurses/man_alias.sed"
	fi
	...
	aliases=`sed -f $top_srcdir/man/manlinks.sed $inalias |sed -f /usr/jens/fix/ncurses/man_alias.sed | sort -u`
	...
	sed	-f /usr/jens/fix/ncurses/man_alias.sed \
		< $i >$TMP
