 ...
 *
 * DO NOT EDIT -- configure objc-3.2.11 i386-unknown-plan9
 */
...
STR objsuffix = "o";	/* o/obj for UNIX/DOS */
STR libsuffix = "a";	/* a/lib for UNIX/DOS */
STR cppsuffix = "P";	/* .P sometimes .i if it's hardcoded */
STR dotobjsuffix = ".o";	/* o/obj for UNIX/DOS */
STR dotlibsuffix = ".a";	/* a/lib for UNIX/DOS */
STR dotshlibsuffix = ".so"; /* _s.lib or .so DOS/UNIX */
STR dotdllsuffix = ".dll";
BOOL uselflag = 1;

STR ccminuscflag = "-c ";	/* -c on UNIX, empty on WATCOM/MPW */
STR ccminusoflag = "-o ";	/* -o on UNIX, -fo= on WATCOM */
STR ldminusoflag = "-o ";	/* -o on UNIX, /OUT: on MSVC */
STR ccminusiflag = "-I";	/* -I on UNIX, -i= on WATCOM */
STR ccminusdflag = "-D";	/* -D on UNIX, -d  on MPW */
STR cppminusoflag = "> ";	/* > on UNIX, -fo= on WATCOM */

STR picflag = "-fpic";
STR staticflag = "-static";
STR picocargs = "";
STR dlargs = "-bogus";
STR dlfile = "objcdlso.ld";
STR dlxldargs = "";
STR dlxccargs = "";

STR linkformat = "unix";	/* unix or watcom */
...
dfltobjcdir(void)
{
  ...
  setstring(&objcdir, "objcdir", [String str:(s) ? s : "/usr/local"]);
}
...
dfltbins()
{
  ...
  s = getenv("CC");
  cc = ([String str:(s) ? s : "cc"]);

  s = getenv("CCPLUS");
  cxx = ([String str:(s) ? s : ""]);

  s = getenv("CPP");
  cpp = ([String str:(s) ? s : "cc -E"]);

  s = getenv("LD");
  ld = ([String str:(s) ? s : "cc"]);
...
dfltargs(void)
{
  ...
  ocargs = addifnonempty([OrdCltn new], "-gnu -noTags");
...
pversion(void)
{
  printf("Portable Object Compiler - objc-3.2.11 - i386-unknown-plan9\n");
...
unlinkfile(id file)
{
  ...
  id cmd = [String sprintf:"rm -f %s", [file str]];
