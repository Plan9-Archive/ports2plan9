/* This file contains a standalone wcwidth() implementation. */
...
static inline int is_nonspacing(unsigned int c)
{
	int ind = nonspacing_table_ind[c >> 9];
	if (ind >= 0)
		return (nonspacing_table_data[64*ind + ((c >> 3) & 63)]
			>> (c & 7)) & 1;
	return 0;
}
...
static inline int is_doublewidth(unsigned int c)
{
	if (c >= 0x1100
...
/* Returns the number of screen columns occupied by the Unicode character c. */
/* For control characters, returns 0. */
int wcwidth(unsigned int c)
{
	if (c < 0x10000) {
		if (is_nonspacing(c))
			return 0;
