...
#if defined(LIBUTF8_PLUG) && defined(HAVE_VSPRINTF)
#define sprintf local_sprintf
static int sprintf (char* s, const char* format, ...)
{
  va_list args;
  int result;

  static int (*original_vsprintf) (char*, const char*, va_list) = NULL;
  if (original_vsprintf == NULL) {
    void* libc_handle = utf8_libc_handle();
    if (libc_handle != NULL)
      original_vsprintf = dlsym(libc_handle,"vsprintf");
  }
  if (!original_vsprintf) {
    const char* errmessage = "libutf8: symbol 'vsprintf' not found\n";
    write(2, errmessage, strlen(errmessage));
    exit(1);
  }

  va_start(args,format);
  result = original_vsprintf(s,format,args);
  va_end(args);
  return result;
}
#endif
...

  {
    ...
    for (cp = format, i = 0, dp = &d.dir[0]; ; cp = dp->dir_end, i++, dp++) {
      ...
      if (dp->conversion == '%') {
        ...
        *s = '\0'; /* for checking whether sprintf returns the count via %n */
        count = 0;
        if (dp->arg_index >= 0) {
          arg_type type = a.arg[dp->arg_index].type;
          switch (type) {
            case TYPE_SCHAR:
              {
                ...
                switch (prefix_count) {
                  case 0: sprintf(s,buf,arg,&count); break;
                  case 1: sprintf(s,buf,prefixes[0],arg,&count); break;
                  case 2: sprintf(s,buf,prefixes[0],prefixes[1],arg,&count); break;
                  default: abort();
                }
              }
              break;
            ...
            case TYPE_WIDE_CHAR:
              {
                ...
                if (n >= 0) {
                  ...
                  *p = 's';
                  switch (prefix_count) {
                    case 0: sprintf(s,buf,mbbuf,&count); break;
                    case 1: sprintf(s,buf,prefixes[0],mbbuf,&count); break;
                    case 2: sprintf(s,buf,prefixes[0],prefixes[1],mbbuf,&count); break;
                    default: abort();
                  }
                } else {
                  errno = EILSEQ;
            ...
            case TYPE_WIDE_STRING:
              {
                ...
                if (n >= 0) {
                  ...
                  switch (prefix_count) {
                    case 0: sprintf(s,buf,mbsarg,&count); break;
                    case 1: sprintf(s,buf,prefixes[0],mbsarg,&count); break;
                    case 2: sprintf(s,buf,prefixes[0],prefixes[1],mbsarg,&count); break;
                    default: abort();
                  }
                  freea(mbsarg);
                } else {
          ...
          switch (prefix_count) {
            case 0: sprintf(s,buf,&count); break;
            case 1: sprintf(s,buf,prefixes[0],&count); break;
            case 2: sprintf(s,buf,prefixes[0],prefixes[1],&count); break;
            default: abort();
          }
        }
        if (*(s+count) != '\0') {
          const char* errmessage = "utf8_vsprintf: sprintf doesn't understand the %n directive\n";
          write(2, errmessage, strlen(errmessage));
          exit(1);
        }
        s += count;
